          ...
          <div className="sm:w-6/12 order-last sm:order-first">
            <ul className="grid grid-cols-2 gap-5 md:gap-10">
              {teamMembers.map((member, index) => (
                <li key={index} className="bg-gray-100 p-5 py-10 text-center transition transform hover:shadow-2xl hover:scale-105 duration-300 ease-in-out">
                  <div className="flex flex-col items-center">
                    <div className="flex-shrink-0">
                      <img className="mb-3 rounded-full mx-auto h-24 w-24" src={member.imgSrc} alt={`${member.name} profile`} />
                    </div>
